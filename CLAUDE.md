...
| `cmd/lplex/` | Multi-subcommand CLI: `dump` (streaming/replay), `inspect` (journal inspection), `verify` (journal integrity verification), `devices`, `values`, `send`, `request`, `switches`. Uses cobra. Pretty-print, device table, PGN decoding (`--decode`), change tracking (`--changes`), display filter expressions (`--where`), auto-reconnect. |
