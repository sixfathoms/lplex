...
| `requestrules/` | `Engine`, `Rule`, `Match`, `Want`, `Request`, `DeviceView`. Declarative on-demand polling: send an ISO Request or parameterized frame when a matching device (by manufacturer/model/class/function/NAME/src/bus) comes online or at startup, keep the datum fresh (`MaxAge`), and re-request on trigger PGNs (`InvalidateOn`). Level-triggered with a per-target `MinInterval` floor + optional global floor so the bus is never flooded. Transport-agnostic (no import of root `lplex`); the broker drives it. |
...
| `pgn/` | Generated Go types for NMEA 2000 PGNs: structs, `Decode*`/`Encode` methods, `Registry` map (~135 entries, includes `PGNInfo` with `FastPacket`, `Interval`, `OnDemand`, `Draft`, `Tolerances` metadata). Name-only PGNs have `Decode: nil` but still carry description and metadata. Generated from `pgn/defs/*.pgn` via `go generate`. Generated output files (`pgn_gen.go`, `helpers_gen.go`, `proto/pgn.proto`, `schema.json`) are gitignored and must be regenerated after DSL changes. Hand-written helpers live alongside generated code (e.g. `ac_utility.go` for AC power PGNs 65009-65016, `diagnostics.go` for J1939 FMI text + combined-SPN/severity helpers on PGN 130762 and Victron charger-error text used by `lplex alarms`, `garmin.go` for Garmin proprietary PGN 126720 route navigation — `GarminActiveWaypoint` sub-type (0x61,0x06) and `GarminDestinationNav` sub-type (0x12,0x07), reverse-engineered from a GPSMAP and registered onto `Registry[126720]` via `init()`). These hand-written decoders are NOT in the generated `proto/pgn.proto`/`schema.json` (those come only from the DSL). AIS PGNs with field definitions: 129038 (Class A Position), 129039 (Class B Position), 129041 (AtoN), 129793 (UTC/Date), 129809 (Static Part A), 129810 (Static Part B). `defs/rvc.pgn` holds the RV-C and vendor-proprietary J1939 PGNs emitted by Wakespeed/Balmar WS500 alternator regulators (RV-C charger status 131015/130723/130969, ISO diagnostics 130762, J1939 EEC1 61444, vendor PGNs 65237/64520) — these reuse NMEA 2000 PGN numbers but follow the RV-C/J1939 field dictionaries, validated against live WS500 frames. Table-driven packet tests in `packets_test.go` verify decode/encode against reference hex data (capture from `lplex dump --decode --json`). |
...
lplex-server supports SIGHUP for hot config reload without restarting. On SIGHUP, it re-reads the HOCON config file and applies: send rules (`send.enabled`, `send.rules`), API key (`api-key`), read-only mode (`read-only`), and rate limits (`send.rate-limit`, `send.rate-burst`). Usage: `kill -HUP $(pidof lplex-server)` or `systemctl reload lplex-server`.
...
lplex-server supports declarative on-demand polling (the `requestrules` package) via HOCON `requests` (array of rule objects). Each rule has a `match` object (any of `manufacturer`, `manufacturer-code`, `model-id` with trailing-`*` glob, `device-class`, `device-function`, `name`, `source`, `bus`; omit = any), a `want` array (PGNs for `via = "iso"`, or sub-key values for `via = "frame"`), timing (`min-interval` required floor between requests for the same datum, optional `max-age` to refresh, `on-online` to fire when the device appears/at startup, `invalidate-on` PGN list to mark stale and re-request), and for `via = "frame"`: `frame-pgn`, `frame-template` (hex), and `subkey-{write,read}-{offset,len}`. A top-level `requests-global-min-interval` caps the rate across all rules. Parsed by `parseRequestRule` in `cmd/lplex-server/config.go`, wired into `BrokerConfig.RequestRules`; embedders use `Broker.AddRequestRule` / `BrokerConfig.RequestRules` directly. The engine is level-triggered (request when missing/stale, never on every event) so combined with `min-interval` it can't flood the bus; disabled in replica mode (no CAN bus).
