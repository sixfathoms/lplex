...
func GenerateGo(s *Schema, pkg string) string {
	...
	for _, p := range s.PGNs {
		...
		writeLookupNamesMethod(&b, p, lookupMap)
	}
...
// lookup names for all lookup fields. The key is the JSON field name, the value is the
// human-readable name (empty string if unknown). Display code uses this to wrap lookup
// fields as {"id": <raw>, "name": "..."} objects in JSON output.
func writeLookupNamesMethod(b *strings.Builder, p PGNDef, lookupMap map[string]*LookupDef) {
	var fields []FieldDef
	for _, f := range p.Fields {
	...
	for _, f := range fields {
		...
		fmt.Fprintf(b, "\t\t%q: %s[m.%s],\n", jsonName, varName, toPascal(f.Name))
	}
	b.WriteString("\t}\n")
	b.WriteString("}\n\n")
}
