...
func TestDecodeVictronSOC(t *testing.T) {
	...
	result, err := decodeProprietarySingleFrame(raw)
	if err != nil {
		t.Fatalf("decode: %v", err)
	}
	m, ok := result.(VictronRegister)
	if !ok {
		t.Fatalf("expected VictronRegister, got %T", result)
	}
...
func TestDecodeProprietaryUnknownManufacturer(t *testing.T) {
	...
	if m.Data != "aabbccddeeff" {
		t.Errorf("Data = %q, want %q", m.Data, "aabbccddeeff")
	}
}
